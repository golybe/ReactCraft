...
const HeldItem = ({ selectedBlock, lightLevel = 15, lastPunchTime, isFlying, isMining = false }) => {
  ...
  useEffect(() => {
    ...
    const loadTex = (name) => {
      const url = getBlockTexture(name);
    ...
    const createMat = (map) => new THREE.MeshBasicMaterial({
      map,
    ...
    if (info.all) {
      const map = loadTex(info.all);
      setMaterials(createMat(map));
    } else {
      const top = loadTex(info.top);
      const side = loadTex(info.side);
      const bottom = loadTex(info.bottom);

      const matSide = createMat(side);
      const matTop = createMat(top);
      const matBottom = createMat(bottom);

      setMaterials([
        matSide, matSide, matTop, matBottom, matSide, matSide
      ]);
    }
    
    lastLightLevel.current = -1;
  }, [selectedBlock, isHand]);
