...
const getItemTexture = (textureName) => {
  if (!itemTextureCache.has(textureName)) {
    ...
    if (source) {
      ...
      itemTextureCache.set(textureName, texture);
    }
  }
  return itemTextureCache.get(textureName);
};
...
const DroppedItem = ({ 
  ...
  const material = useMemo(() => {
    ...
    const textureName = textureInfo?.all || textureInfo?.side || textureInfo?.top || block.texture;
    const texture = textureName ? getItemTexture(textureName) : null;
