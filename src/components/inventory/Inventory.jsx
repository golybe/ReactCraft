...
const CreativeInventory = ({
  ...
  const handlePaletteClick = (blockId) => {
    ...
    // Creative: give full stack of 64, but tools only 1
    const count = block?.isTool ? 1 : MAX_STACK_SIZE;
    setCursorItem({ type: blockId, count });
  };

  const handlePaletteRightClick = (blockId) => {
    const block = BlockRegistry.get(blockId);
    // Same as left click in palette
    const count = block?.isTool ? 1 : MAX_STACK_SIZE;
    setCursorItem({ type: blockId, count });
  };
      ...
      >
        {cursorData && (
          <>
            ...
            {/* Hide count in creative if it's a full stack, or if showCount is false */}
            {cursorData.count > 1 && !(isCreativeMode && cursorData.count === MAX_STACK_SIZE) && (
