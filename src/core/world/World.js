...
import { TileEntityManager } from '../tileentity';
import { CHUNK_SIZE, CHUNK_HEIGHT } from '../../constants/world';
import { BLOCK_TYPES } from '../../constants/blocks';

export class World {
  constructor(seed, savedData = {}) {
    this.seed = seed;

    // Поддержка старого формата (где savedData это просто chunks)
    const savedChunks = savedData.chunks || (savedData.tileEntities ? {} : savedData);
    const savedTileEntities = savedData.tileEntities || [];
    ...
    // Инициализируем TileEntityManager
    if (savedTileEntities.length > 0) {
      TileEntityManager.deserialize(savedTileEntities);
    } else {
      TileEntityManager.clear();
    }
    ...
    // Настраиваем TileEntityManager для работы с освещением
    this.setupTileEntityCallbacks();
  }

  /**
   * Настраиваем TileEntityManager для работы с системой освещения
   */
  setupTileEntityCallbacks() {
    // Callback для добавления/удаления источника света от TileEntity
    TileEntityManager.setLightingCallback((worldX, worldY, worldZ, lightLevel, isAdding) => {
    ...
    TileEntityManager.setChunkUpdateCallback((affectedChunks) => {
  ...
  setBlock(x, y, z, blockType, metadata = 0) {
    ...
    if (success) {
      ...
      // Удаляем TileEntity если разрушен/заменен соответствующий блок
      if (oldBlockType !== blockType && TileEntityManager.isTileEntityBlock(oldBlockType)) {
        TileEntityManager.remove(x, y, z);
      }
  ...
  getSaveData() {
    return {
      chunks: this.chunkManager.getSaveData(),
      tileEntities: TileEntityManager.serialize()
    };
  }
